...
**To keep the 3.x layout,** turn on `useLegacyInteractionLayer()`. The container shrinks back to the icon size and only the press feedback layer spills outside it, so a larger container never forces you to retune the surrounding spacing.

```swift
// 3.x - the number is the icon size
IconButton(variant: .normal(size: 22), icon: .search)

// 4.0 - same meaning, same footprint as 3.x
IconButton(variant: .normal(size: .custom(size: 22)), icon: .search)
    .useLegacyInteractionLayer()
```

On a named size, the 4.0 icon size becomes the container.

- `.small` - icon and container 16, interaction layer 24, radius 8
- `.medium` - 18, 28, 8
- `.large` - 20, 32, 10
- `.xlarge` - 24, 36, 10

On `.custom(size:)` the number is both the icon and the container, clamped to `[12, 64]`. The interaction layer is `max(24, size, size ÷ (2/3) snapped to a dimension token)`, and the radius is that layer size × 0.3 snapped to a radius token. Other variants ignore the modifier.
...
The icon size (24) and the space each button occupies stay as they were in 3.x, via `useLegacyInteractionLayer()`. The bar's horizontal padding and the gaps between icons do not change.
...
| `IconButton` | `interactionEffect(_:)`, `interactionColor(_:)`, `useLegacyInteractionLayer(_:)` |
...
- [ ] `grep -rn "normal(size: \.custom("` - if a 3.x icon size was carried over verbatim, switch it to a container value or turn on `useLegacyInteractionLayer()`
...
- [ ] Icon spacing on screens that use `IconButton` directly - if the larger container pushed them apart, turn on `useLegacyInteractionLayer()`

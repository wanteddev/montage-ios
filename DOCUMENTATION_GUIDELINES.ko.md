...
## 9. docstring 내 마크다운 인라인 서식

DocC는 docstring을 Markdown(GFM)으로 파싱하며, `**볼드**`·`*이탤릭*`·심볼 링크(` ``TypeName`` `)·취소선은 생성된 md에 그대로 보존됩니다. 의도적으로 쓰는 것은 문제없지만, **의도치 않게 서식 문자로 해석되는 경우**를 주의해야 합니다.

### 🚨 중요: `~`(틸드)는 취소선으로 해석됨 — 범위 표기 시 반드시 이스케이프

DocC 파서는 **단일 `~`도 취소선 문법**으로 인식합니다. 한 문단에 `~`가 2개 있으면 둘이 짝지어져 그 사이 텍스트가 취소선 처리되어 문서가 깨집니다. 가장 흔한 함정은 `~`를 **숫자 범위 구분자**로 쓰는 경우입니다.

```swift
// ❌ 깨짐 — 한 줄에 ~가 2개라 "90%, 마지막 줄 40"이 취소선으로 묶임
/// 중간 줄 65~90%, 마지막 줄 40~55% 비율로 생성합니다.

// ✅ 백슬래시로 틸드를 이스케이프 (DocC가 리터럴 ~로 처리)
/// 중간 줄 65\~90%, 마지막 줄 40\~55% 비율로 생성합니다.
...
- **범위 표기의 `~`는 항상 `\~`로 이스케이프**합니다. 한 줄에 `~`가 하나뿐이면(예: `24\~64pt`) 당장은 안 깨지지만, 나중에 같은 문단에 범위가 추가되면 깨지므로 예외 없이 이스케이프하는 것을 규칙으로 합니다.
- 의도적인 취소선이 아니라면 `~`/`~~`가 docstring에 그대로 남지 않도록 합니다.

### 백스톱: 변환기가 취소선을 감지하면 경고

`scripts/docc_to_md.js`는 취소선 노드를 만나면 경고를 출력합니다. Montage는 취소선을 의도적으로 쓰지 않으므로, `make` 실행 로그에 아래 경고가 보이면 해당 docstring의 `~`를 이스케이프하세요.
...
6. docstring의 범위 구분자 `~`를 `\~`로 이스케이프했는지 확인 (취소선 오인 방지)
7. 스크립트 실행 로그에서 에러/경고 메시지 확인

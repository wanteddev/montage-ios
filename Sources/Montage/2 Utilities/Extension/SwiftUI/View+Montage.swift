...
extension View {
    ...
    ///
    /// - Parameter action: 크기가 .zero로 변경되거나 .zero가 아닌 값으로 변경될 때 실행할 액션 클로져. 파라메터로는 View 크기가 .zero인지 여부가 전달됩니다.
    /// - Returns: 수정된 View
    /// - Note: opacity(0), hidden() 등 시각적으로 비어 보이지만 사이즈를 가지는 케이스는 감지되지 않습니다.

...
/// ``Montage/RadioUIView``의 터치 이벤트를 받을 수 있는 Delegate입니다.
protocol RadioControlDelegate: AnyObject {
    /// 터치가 발생하였을 때 호출되는 메소드입니다.
    /// - Parameter radio: 터치가 발생한 객체
    func didTappedRadio(_ radio: Control.RadioUIView)
}
...
extension Control {
    ...
    public final class RadioUIView: UIView, MontageUIControl {
        ...
        public var state: MontageControlState = .unchecked {
            didSet {
                if state == .indeterminate {
                    #warning("Can't use .indeterminate type on RadioUIView element. Must use .checked type.")
                    state = .checked
                }
                updateViews()
            }
        ...
        /// RadioUIView 객체를 생성합니다.
        /// - Parameter delegate: RadioUIView 버튼을 Element 단독으로 사용할 경우 이벤트를 받을 delegate 객체입니다.
...
extension Control.RadioUIView: UIGestureRecognizerDelegate {

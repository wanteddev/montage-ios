...
/// ``Montage/CheckboxUIView``의 터치 이벤트를 받을 수 있는 Delegate입니다.
protocol CheckboxControlDelegate: AnyObject {
    /// 터치가 발생하였을 때 호출되는 메소드입니다.
    /// - Parameter checkbox: 터치가 발생한 객체
    func didTappedCheckbox(_ checkbox: Control.CheckboxUIView)
}
...
extension Control.CheckboxUIView {
...
extension Control.CheckboxUIView {
    ...
    var boxInsets: UIEdgeInsets {
        switch size {
        case .normal:
            .init(top: 3, left: 3, bottom: 3, right: 3)
        case .small:
...
extension Control.CheckboxUIView: UIGestureRecognizerDelegate {

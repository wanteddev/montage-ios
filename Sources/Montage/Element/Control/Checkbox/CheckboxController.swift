...
extension Control {
    ...
    public struct CheckboxController: UIViewRepresentable {
        ...
        private let size: Checkbox.Size
        private let onTap: (UIViewType) -> Void
        private var isDisable: Bool = false
        
        public typealias UIViewType = Checkbox
        
        public init(_ state: Binding<MontageControlState>, size: Checkbox.Size, onTap: @escaping (UIViewType) -> Void = { _ in }) {
            _state = state
            self.size = size
            self.onTap = onTap
        }
        
        public init(state: MontageControlState, size: Checkbox.Size, onTap: @escaping (UIViewType) -> Void = { _ in }) {

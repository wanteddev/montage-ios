...
public enum PullToRefresh {
    struct Modifier: ViewModifier {
        ...
        private var pullDownRatio: CGFloat {
            max(0, scrollYOffset - animationThreshold)
                / max(1, refreshThreshold - animationThreshold)
        }
    ...
    private struct AnimatingLogo: View {
        ...
        @State private var playbackMode: LottiePlaybackMode = .playing(
            .toMarker("Loop", loopMode: .playOnce))
...
extension View {
    ...
    public func pullToRefresh(
        ...
        modifier(PullToRefresh.Modifier(scrollYOffset: scrollYOffset, refresh: refresh))
    }
}

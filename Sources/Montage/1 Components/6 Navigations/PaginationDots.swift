...
///
/// `PaginationDots`은 페이지 간 이동 및 현재 페이지 표시를 위한 점 형태의 페이지네이션 컴포넌트를 제공합니다.
...
///     .size(.normal)
/// ```
public struct PaginationDots: View {
    ...
    private struct Element: View {
        ...
        private var dotColor: SwiftUI.Color {
            switch variant {
            case .normal:
                .semantic(.foregroundNeutralPrimary).opacity(selected ? 1 : .opacity16)
            case .white:
                .semantic(.staticWhite).opacity(selected ? 1 : .opacity52)
            }
        }
        ...
        private var dotBorderColor: SwiftUI.Color {
            ...
            case .normal:
                .clear
            case .white:
                .semantic(.lineNeutralSecondary).opacity(selected ? 1 : .opacity52)
            }
        }
    }
}

...
public struct SegmentedControl: View {
    ...
    /// 이 경우 각 ``Item``에 이미지를 지정해야 하며, ``Item``의 `title`은 세그먼트의 VoiceOver 접근성 라벨로 사용됩니다.
    ///
...
extension SegmentedControl {
    ...
    ///
    /// 접근 제한자가 `internal`인 이유는 `@testable import`를 통해 단위 테스트에서 검증하기 위함입니다.
    var buttonIconSize: CGSize {
        if iconOnly {
            switch size {
            case .large:

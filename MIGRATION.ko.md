...
**3.x 레이아웃을 그대로 쓰려면** `useLegacyInteractionLayer()`를 켭니다. 컨테이너가 아이콘 크기까지 줄고 press 피드백 레이어만 그 밖으로 번지므로, 커진 컨테이너 때문에 주변 간격을 다시 잡을 필요가 없습니다.

```swift
// 3.x - 숫자가 아이콘 크기
IconButton(variant: .normal(size: 22), icon: .search)

// 4.0 - 숫자의 의미도 차지하는 자리도 3.x와 같아진다
IconButton(variant: .normal(size: .custom(size: 22)), icon: .search)
    .useLegacyInteractionLayer()
```

named size에 켜면 4.0의 아이콘 크기가 그대로 컨테이너가 됩니다.

- `.small` - 아이콘 겸 컨테이너 16, 인터랙션 레이어 24, radius 8
- `.medium` - 18, 28, 8
- `.large` - 20, 32, 10
- `.xlarge` - 24, 36, 10

`.custom(size:)`에 켜면 숫자가 곧 아이콘이자 컨테이너이고 `[12, 64]`로 clamp됩니다. 인터랙션 레이어는 `max(24, size, size ÷ (2/3)을 dimension 토큰에 스냅한 값)`, radius는 그 레이어 크기 × 0.3을 radius 토큰에 스냅한 값입니다. `normal` 외의 variant에서는 무시됩니다.
...
아이콘 크기(24)와 버튼이 차지하는 자리는 `useLegacyInteractionLayer()`로 3.x와 같게 유지됩니다. 내비게이션 바의 좌우 여백과 아이콘 간격은 달라지지 않습니다.
...
- [ ] `grep -rn "normal(size: \.custom("` - 3.x의 아이콘 크기를 그대로 옮겼다면 컨테이너 값으로 바꾸거나 `useLegacyInteractionLayer()`를 켰는지
...
- [ ] IconButton을 직접 쓰던 화면의 아이콘 간격 - 컨테이너가 커진 만큼 벌어졌다면 `useLegacyInteractionLayer()`
